...
class Discoteca(models.Model):
    ...
    """
    name = models.OneToOneField(Artista, on_delete=models.SET_NULL, blank=True, null=True)
...
class Album(models.Model):
    ...
    Represents an album
    """
    discoteca = models.ForeignKey(Discoteca, on_delete=models.CASCADE, null=True, blank=True)
    ...
    def formatHelper(self, matchobj):
        url = static(settings.MEDIA_URL +
                     'archive/Discografias/'+matchobj.group(1))
        return r'<audio controls preload="none"><source src="' + url + \
            '" type="audio/mpeg">Su explorador es antiguo. Actualicelo para reproducir audios.</audio>'
...
class Video(models.Model):
    ...
    link = models.FileField('Video', upload_to='videos/',
                            validators=[validate_file_extension], default='')
